...
const artistsExemple = [
  "Picasso",
  { nameExemple: "Van Gogh", artExemple: "Starry Night" },
  "Monet",
];

const [b1, b2, b3] = artistsExemple;

console.log(b1);

const { nameExemple, artExemple } = b2;
console.log(nameExemple);
console.log(artExemple);

console.log(b3);
// The next line does not work as expected, uncomment & fix it!
chooseBetween(artists[0], artists[1]);
...
import Laptop from "./stuff.js";
import { isAppCompatible } from "./stuff.js";

const myLaptop = new Laptop("Apple", "MacBook Pro", 15.6, "16GB", "512GB SSD");

console.log(
  `My laptop: ${myLaptop.brand} ${myLaptop.model} with ${myLaptop.ram} RAM`
);

myLaptop.upgradeRAM("32GB");
console.log(`Upgraded RAM to ${myLaptop.ram}`);

const appCompatibility = isAppCompatible("Photoshop", 8);
console.log(`Is Photoshop compatible? ${appCompatibility ? "Yes" : "No"}`);
...
const numberOfFriends = people.map((person) => person.friends.length);
console.log(numberOfFriends);

const emailDomains = people.map((person) => {
...
const favoriteFruits = [
  ...new Set(people.flatMap((person) => person.favoriteFruit)),
].sort();
console.log(favoriteFruits);

const averageAge = people.reduce((acc, person) => acc + person.age, 0);
const totalAge = averageAge / people.length;
console.log(totalAge);

const totalBalance = people.reduce((acc, person) => {
  const balanceNumber = parseFloat(
    person.balance.replace("$", "").replace(",", "")
  );
  return acc + balanceNumber;
}, 0);
const averageBalance = totalBalance / people.length;
const richPeople = averageBalance > 2500;
